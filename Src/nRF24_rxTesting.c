...
//	while(!(RF24_sendInstruction(SPI2, RF24_ADDR_STATUS, dummy)[1] & (1 << RF24_RX_DR)));

	// here I could turn RXs into an uint8_t and just assign [1] of the send instruction to it, no then i can't free memory
	uint8_t* RXs = NULL;
	while(1)
	{
		RXs = NULL;
		RXs = RF24_sendInstruction(SPI2, RF24_ADDR_STATUS, dummy);
		if(RXs[1] & (1 << RF24_RX_DR))
		{
			free(RXs);
			break;
		}
...
//  arduino ex
	...
	RF24_init(SPI2, GPIOB, GPIO_PIN_NO_10);
	local_delay(1500);
...
//	local_delay(1500);
	...
	GPIOB->ODR |= (1 << GPIO_PIN_NO_10); // set high (rx mode)
	local_delay(1500);
